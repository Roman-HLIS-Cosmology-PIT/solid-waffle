...
def read_summary(fname):
    """Extract summary data from file for data table in Freudenburg et al. (2020) paper."""
    ...
    table["alpha"] = (2 * table["alphaH"] + 2 * table["alphaV"] + 4 * table["alphaD"]) / 8.0

    table["ipnl<1,0>"] = (
        table["ipnl(0,1)"] + table["ipnl(0,-1)"] + table["ipnl(1,0)"] + table["ipnl(-1,0)"]
    ) / 4.0
    table["ipnl<1,1>"] = (
        table["ipnl(1,1)"] + table["ipnl(1,-1)"] + table["ipnl(-1,1)"] + table["ipnl(-1,-1)"]
    ) / 4.0
    table["ipnl<2,0>"] = (
        table["ipnl(0,2)"] + table["ipnl(0,-2)"] + table["ipnl(2,0)"] + table["ipnl(-2,0)"]
    ) / 4.0
    table["ipnl<2,1>"] = (
        table["ipnl(2,1)"]
        + table["ipnl(2,-1)"]
    ...
    table["ipnl<2,2>"] = (
        table["ipnl(2,2)"] + table["ipnl(2,-2)"] + table["ipnl(-2,2)"] + table["ipnl(-2,-2)"]
    ) / 4.0
