...
S.hist(
    vis_out_data[:, :, 51].ravel(), num_bins, histtype="step", label=r"$C_{11}$", linewidth=1.5, linestyle="-"
)
S.hist(
    vis_out_data[:, :, 52].ravel(),
    bins=np.linspace(-0.1, 0.1, num=num_bins),
    histtype="step",
    label=r"$C_{12}$",
    linewidth=1.5,
    linestyle="--",
