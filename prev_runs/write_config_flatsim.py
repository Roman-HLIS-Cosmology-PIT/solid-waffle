...
if flatflag == "flat":
    file = open(  # legacy function # noqa: SIM115
        "%s/%ssim_config_%s_rng%d" % (tdir, waveflag, name, int(rng)), "w"
    )
else:
    file = open(  # legacy function # noqa: SIM115
        "%s/%ssim_config_%s_%s_rng%d" % (tdir, waveflag, name, flatflag, int(rng)), "w"
    )
