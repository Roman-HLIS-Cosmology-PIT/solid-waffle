...
for line in content:
    ...
    if m:
        configInfo.label = m.group(1)
    m = re.search(r"^SCA\:\s*(\d+)", line)
    if m:
        configInfo.sca = int(m.group(1))
    m = re.search(r"^IN\:\s*(\S*)", line)
    if m:
        configInfo.IN = m.group(1)
    m = re.search(r"^OUT\:\s*(\S*)", line)
    if m:
        configInfo.OUT = m.group(1)
    m = re.search(r"^NOISE\:\s*(\S*)", line)
    if m:
        configInfo.NOISE = m.group(1)
    ...
    if m:
        configInfo.PersistScript = m.group(1)
...
# summary information
with open(configInfo.IN) as myf:
    summaryinfo = myf.read().splitlines()
summaryMetadata = []
colData = []
countCol = False
for line in summaryinfo:
    ...
    if m:
        ...
        if mm:
            colData.append(mm.group(1))
        if line[2:] == r"Columns\:":
            countCol = True
        if not countCol:
            summaryMetadata.append(line[2:])

nx = 1 + int(numpy.max(summaryData[:, 0]))
ny = 1 + int(numpy.max(summaryData[:, 1]))
print(f"Main file grid: nx = {nx:d}, ny = {ny:d}")

col = pyirc.IndexDictionary(0)
sbfe = 2
col.addbfe(sbfe)
p = len(colData) - col.N - 2
print("number of nl coefficients", p)
col.addhnl(p)
...
SuperPixelMask = (
    numpy.concatenate((SuperPixelMask[0], maskY)).astype(numpy.int64),
...
for imask in range(nmask):
    ...
    dy = 4096 // ny
    dx = 4096 // nx
    badpix[iy * dy : (iy + 1) * dy, ix * dx : (ix + 1) * dx] = numpy.bitwise_or(
        0x40, badpix[iy * dy : (iy + 1) * dy, ix * dx : (ix + 1) * dx]
    )
...
for mask_index in range(len(maskX)):
    ix = maskX[mask_index]
    iy = maskY[mask_index]
    this_metadata.append(f"manual mask superpix iy={iy:3d} ix={ix:3d}")
    print(f"manual mask superpix iy={iy:3d} ix={ix:3d}")
...
# flattened BFE image
bfeflat_hdu = fits.ImageHDU(
    numpy.transpose(bfeData, (0, 2, 1, 3)).astype(numpy.float32).reshape(ssbfe * ny, ssbfe * nx)
)
...
if visdata:
    ...
    lmax = 2.4
    for k in range(nl):
        linv = 1.0 / lmax + k / (nl - 1) * (1.0 / lmin - 1.0 / lmax)  # 1/lambda in inverse microns
        if linv > 1.25:
            qy[k, :, :] = 0.04 * (linv - 1.25)
...
ipc_auto[1, 1, :, :] = 1.0 - sum_ipc
ipc_full = numpy.copy(ipc_auto)

# Placeholder for VTPE
vtpe_good = False
vtpe = numpy.zeros((3, 512, 512))
vtpe[2, :, :] = 1.0
...
if configInfo.use_spr:
    ...
    for iy in range(ny):
        for ix in range(nx):
            for ky in range(3):
                for kx in range(3):
                    ipc_full[ky, kx, iy * sy : (iy + 1) * sy, ix * sx : (ix + 1) * sx] = ipc_auto[
                        ky, kx, iy, ix
                    ]
    ...
    for j in range(configInfo.NSPR):
        SPRFile.append(st + f"{en:02d}_alpha.fits")
        this_metadata.append("Used SPR file: " + SPRFile[j])
        with fits.open(SPRFile[j]) as G:
            for keyword in G[0].header:
                m = re.search(r"^ARGV", keyword)
                if m:
                    this_metadata.append("  " + keyword + " -> " + G[0].header[keyword])
                m = re.search(r"^INF", keyword)
                if m:
                    this_metadata.append("  " + keyword + " -> " + G[0].header[keyword])
            medsig.append(float(G[0].header["MEDSIG"]))
            alphadata[j, :, :, :] = G[0].data
        print(" median signal:", medsig[j], "DN")
        this_metadata.append(" median signal: " + str(medsig[j]) + " DN")
        en += configInfo.dSPR
    ...
    s = 9
    s_th = 0.01  # threshold for clipping
    ...
    s_th = 0.001  # threshold for clipping
    ...
    vtpe[1, :, :] = numpy.minimum(
        (vt_big - vt_small) / numpy.log(Q_big / Q_small),
        -1.0e-24 * numpy.ones((512, 512), dtype=numpy.float64),
    )  # to guarantee not zero
    ...
    for iy in range(ny):
        for ix in range(nx):
            thisfloor[iy, ix] = 2 * (
                numpy.average(this_aV[iy * dy : (iy + 1) * dy, ix * dx : (ix + 1) * dx]) - aV[iy, ix]
            )
    thisfloor = ndimage.median_filter(thisfloor, size=(3, 1), mode="reflect")
    for iy in range(ny):
        for ix in range(nx):
            xicpt = numpy.log(
                numpy.average(Q_small[iy * dy : (iy + 1) * dy, ix * dx : (ix + 1) * dx])
            ) - numpy.average(
                vt_small[iy * dy : (iy + 1) * dy, ix * dx : (ix + 1) * dx] - thisfloor[iy, ix]
            ) / numpy.average(vtpe[1, iy * dy : (iy + 1) * dy, ix * dx : (ix + 1) * dx])
            print(
                iy,
                ix,
                thisfloor[iy, ix],
                numpy.average(Q_small[iy * dy : (iy + 1) * dy, ix * dx : (ix + 1) * dx]),
                numpy.average(vt_small[iy * dy : (iy + 1) * dy, ix * dx : (ix + 1) * dx]),
                numpy.average(vtpe[1, iy * dy : (iy + 1) * dy, ix * dx : (ix + 1) * dx]),
                xicpt,
            )
            vtpe[2, iy * dy : (iy + 1) * dy, ix * dx : (ix + 1) * dx] = 1.0 + numpy.exp(xicpt)
    vtpe[0, :, :] = vt_big - vtpe[1, :, :] * numpy.log(1.0 + Q_big / vtpe[2, :, :])
...
ipcflat_hdu = fits.ImageHDU(
    numpy.transpose(ipc_full, (0, 2, 1, 3)).astype(numpy.float32).reshape((ay * nyi, ax * nxi))
)
...
med_cdsnoise_DN_ref = numpy.median(
    numpy.concatenate(
        (
            cdsnoise[:, :4].flatten(),
            cdsnoise[:, -4:].flatten(),
            cdsnoise[:4, 4:-4].flatten(),
            cdsnoise[-4:, 4:-4].flatten(),
        )
    )
)
...
#
for noisekey in noisefile["NOISE"].header:
...
for j in range(ny):
    for i in range(nx):
        fullgain[j * (4096 // ny) : (j + 1) * (4096 // ny), i * (4096 // nx) : (i + 1) * (4096 // nx)] = gain[
            j, i
        ]
...
noise_hdu.header["PEDESTAL"] = (4.0, "placeholder")
...
noise_hdu.header["PCA0_AMP"] = noisefile["NOISE"].header["PCA0_AMP"] * numpy.median(
    gain
)  # convert from DN --> e
...
badpix = numpy.bitwise_or(
    numpy.where(noisefile["NOISE"].data[noise_frame_lowcdshightot, :, :] < 0.5, 0, 0x10).astype(badpix.dtype),
    badpix,
)

# Get the bias information
bias_hdu = fits.ImageHDU(
    numpy.clip(noisefile["NOISE"].data[noise_frame_bias, :, :], 0, 65535).astype(numpy.uint16)
)
...
dark_current = numpy.where(
    dark_current < dark_crit1, noisefile["NOISE"].data[noise_frame_dark2, :, :], dark_current
)
...
if configInfo.Dark:
    ...
    for j in range(configInfo.NDarkFile):
        DarkFile.append(st + f"{en:03d}.fits")
    ...
    with fits.open(DarkFile[0]) as G:
        ...
        if td3 < 0.01:
            td3 = float(G[0].header["TFRAME"]) * (1 + float(G[0].header["GROUPGAP"])) * (nt - NFowler)
    darkimage = numpy.zeros((configInfo.NDarkFile, 4096, 4096), dtype=numpy.float32)  # Fowler 4
    for j in range(configInfo.NDarkFile):
        for k in range(NFowler):
            thisdiff = pyirc.load_segment(DarkFile[j], informat, [0, 4096, 0, 4096], [1 + k], False).astype(
                numpy.float32
            ) - pyirc.load_segment(DarkFile[j], informat, [0, 4096, 0, 4096], [nt - k], False).astype(
                numpy.float32
            )
    ...
    dark_var_rate = (
        numpy.median(numpy.absolute(darkimage / td3 - longdark), axis=0).astype(numpy.float32) / 0.67448
    ) ** 2 * td3
    # substitute this dark rate if need be
    dark_crit2 = 256.0 / td2
...
if configInfo.FW:
    ...
    for j in range(configInfo.NFullWellFile):
        FWFile.append(st + f"{en:03d}.fits")
    ...
    tmax = numpy.zeros((4096, 4096), dtype=numpy.int16)
    tmax[:, :] = nt - 1
    ...
    for tm in range(configInfo.NLD + 1, nt)[::-1]:
        ...
        for i in range(configInfo.NLD + 1):
            for j in range(configInfo.NLD + 1):
                A[i, j] = numpy.sum(
                    numpy.array(range(d_offset, tm + d_offset)).astype(numpy.float64) ** (i + j)
                )
            for j in range(tm):
                B[i, j] = (d_offset + j) ** i
        AinvB = numpy.matmul(numpy.linalg.inv(A), B)
        # print('shapes: ', numpy.shape(AinvB), numpy.shape(my_stack))
        these_poly_coefs[:, :, :] = numpy.tensordot(
            AinvB, my_stack[:tm, :, :], axes=([1], [0])
        )  # sum_j Ainv[i,j] * my_stack[j,:,:]
        #
        if tm == nt - 1:
            poly_coefs[:, :, :] = these_poly_coefs  # start by accepting all coefficients
        #
        ...
        if tm < nt - 1:
            ...
            for i in range(configInfo.NLD + 1):
                err -= numpy.tensordot(t**i, these_poly_coefs[i, :, :], axes=0)
            err = numpy.sqrt(numpy.average(err**2, axis=0))
            BadMask = numpy.logical_or(
                my_stack[tm, :, :] - my_stack[tm - 1, :, :]
                < (my_stack[tm - 1, :, :] - my_stack[tm - 2, :, :]) / 2.0,
                err > 327.68,
            )
            der = numpy.zeros((4096, 4096))
            for i in range(1, configInfo.NLD + 1):
                der += poly_coefs[i, :, :] * (d_offset + tmax - 1).astype(numpy.float64) ** (i - 1) * i
            BadMask = numpy.logical_or(BadMask, der < 0)
            print(
                "tm = ",
                tm,
                " array-median RMS err = ",
                numpy.median(err),
                "mask=",
                numpy.sum(numpy.where(BadMask, 1, 0)),
            )
            sys.stdout.flush()
    ...
    badpix[4:-4, 4:-4] = numpy.bitwise_or(
        numpy.where(tmax[4:-4, 4:-4] == configInfo.NLD + 1, 0x20, 0).astype(badpix.dtype), badpix[4:-4, 4:-4]
    )
    #
    for tm in range(1, configInfo.NLD + 1)[::-1]:
        der = numpy.zeros((4096, 4096))
        for i in range(1, configInfo.NLD + 1):
            der += poly_coefs[i, :, :] * float(d_offset + tm) ** (i - 1) * i
        delta = numpy.zeros((4096, 4096))
        for i in range(1, configInfo.NLD + 1):
            delta += poly_coefs[i, :, :] * (float(d_offset + tm) ** i - float(d_offset + tm - 1) ** i)
        tmax = numpy.where(numpy.logical_or(der < 0, delta < 0), tm, tmax)
    del der
    del delta
    # print statistics of the tmax file
    for tm in range(0, nt):
        print(f"{tm:2d} {numpy.sum(numpy.where(tmax==tm,1,0).astype(numpy.int32)):8d}")
    ...
    for iy in range(ny):
        for ix in range(nx):
            sat_level[dy * iy : dy * (iy + 1), dx * ix : dx * (ix + 1)] *= gain[
                iy, ix
    ...
    for tm in range(1, nt):
        ...
        t = numpy.asarray(range(d_offset, d_offset + tm)).astype(numpy.float64)
        for i in range(configInfo.NLD + 1):
            err -= numpy.tensordot(t**i, poly_coefs[i, :, :], axes=0)
        err_level = numpy.where(
            tmax == tm, numpy.sqrt(numpy.mean(err**2, axis=0)).astype(numpy.float32), err_level
        )
    #
    ...
    for q in range(1, configInfo.NLD):
        poly_coefs[1 + q, :, :] = numpy.where(
            poly_coefs[1, :, :] == 0, 0.0, -poly_coefs[1 + q, :, :] / poly_coefs[1, :, :] ** (1 + q)
        )  # write in terms of DN_lin
        dx = 4096 // nx
        dy = 4096 // ny
        for iy in range(ny):
            for ix in range(nx):
                poly_coefs[1 + q, dy * iy : dy * (iy + 1), dx * ix : dx * (ix + 1)] /= (
                    gain[iy, ix] ** q
                )  # convert to electrons
    ...
    flat_field[-4:, :] = 0.0
    flat_field[:, :4] = 0.0
    flat_field[:, -4:] = 0.0  # take out reference pixels
...
# alternative
    ...
    for q in range(p):
        poly_coefs[q, :, :] = (
            numpy.copy(summaryData[:, col.Nbb + 2 : col.Nbb + q + 2].reshape((ny, nx))) / gain**q
        )
...
cnl_hdu.header["ERR50"] = (numpy.percentile(err_level, 50.0), "50th percentile CNL fit error (DN)")
cnl_hdu.header["ERR90"] = (numpy.percentile(err_level, 90.0), "90th percentile CNL fit error (DN)")
cnl_hdu.header["ERR95"] = (numpy.percentile(err_level, 95.0), "95th percentile CNL fit error (DN)")
cnl_hdu.header["ERR99"] = (numpy.percentile(err_level, 99.0), "99th percentile CNL fit error (DN)")
...
for iy in range(ny):
    for ix in range(nx):
        flat2[dy * iy : dy * (iy + 1), dx * ix : dx * (ix + 1)] /= numpy.median(
            flat_field[dy * iy : dy * (iy + 1), dx * ix : dx * (ix + 1)]
        )
weirdmap = numpy.where(numpy.absolute(flat2 - 1) > 0.1, 1, 0)
weirdmap[4:-4, 4:-4] = ndimage.maximum_filter(weirdmap[4:-4, 4:-4], size=3, mode="reflect")
badpix[4:-4, 4:-4] = numpy.bitwise_or(
    badpix[4:-4, 4:-4], numpy.where(weirdmap[4:-4, 4:-4], 8, 0).astype(badpix.dtype)
)
...
if configInfo.PersistScript:
    ...
    with open(configInfo.PersistScript) as myf:
        content = myf.read().splitlines()
    NP = len(content)
    Q = []
    ...
    for f in range(NP):
        ...
        if m:
            ...
            with fits.open(DarkFile[0]) as G:
                ...
                if tgroup < 0.01:
                    tgroup = float(G[0].header["TFRAME"]) * (1 + float(G[0].header["GROUPGAP"]))
                ngroups = float(G[0].header["NGROUPS"])
            ngroups = min(4, ngroups)  # stop after 4th group
            print("<--", fn)
            sys.stdout.flush()
            persistence_map[f, :, :] = pyirc.load_segment(
                fn, informat, [0, 4096, 0, 4096], [1], False
            ).astype(numpy.float32)
            for k in range(2, ngroups + 1):
                persistence_map[f, :, :] = persistence_map[f, :, :] - pyirc.load_segment(
                    fn, informat, [0, 4096, 0, 4096], [k], False
                ).astype(numpy.float32) / (ngroups - 1)
            # reference pixel subtraction
            for i in range(4096):
            ...
            print(f"Error: failed to match line {f:d}: " + content[f])
            exit()
    Q = numpy.asarray(Q)
    ...
    for j in range(NP):
        ...
        persistence_map[j, :, :] = persistence_map[j, :, :] - tgroup * dark_current * (ngroups / 2.0)
        # prevent large over-subtraction by clipping at -60 e
        persistence_map[j, :, :] = numpy.where(
            numpy.logical_and(persistence_map[j, :, :] < -60.0, tgroup * dark_current * (ngroups / 2.0) > 60),
            -60.0,
            persistence_map[j, :, :],
        )
        # convert from e in tgroup to e per ln t
        X = numpy.average(numpy.log(1.0 + tgroup / frtime * numpy.linspace(1, ngroups - 1, ngroups - 1)))
        persistence_map[j, :, :] /= X
    print("average ln (tmax/tmin) =", X, " used ngroups=", ngroups)
    for j in range(NP):
        st = f"pers at {Q[j]:8.1f} :"
        for i in range(1, 10):
            st = st + f" {numpy.percentile(persistence_map[j,:,:], i*10):5.1f}"
        st += " (unfilt)"
        print(st)
        this_metadata.append(st)
    #
    # median filter the image
    print("median filtering persistence images ...")
    nfilt = 5
    for j in range(NP):
        persistence_map[j, 4:-4, 4:-4] = ndimage.median_filter(
            persistence_map[j, 4:-4, 4:-4], size=nfilt, mode="reflect"
        )
    for j in range(NP):
        st = f"pers at {Q[j]:8.1f} :"
        for i in range(1, 10):
    ...
    for j in range(1, NP):
        persist_hdu.header[f"Q{j:02d}"] = (Q[j], "Stimulus in e")
    alpha = numpy.log(
        numpy.median(persistence_map[-1, :, :]) / numpy.median(persistence_map[-2, :, :])
    ) / numpy.log(Q[-1] / Q[-2])
    print("alpha =", alpha)
    ...
    persist_hdu.header["Q01"] = (1e4, "Placeholder")
    persist_hdu.header["Q02"] = (1e5, "Placeholder")
    persist_hdu.header["ALPHA"] = (0.0, "Placeholder")
this_metadata.append("")
...
# Source information
for k in range(len(this_metadata)):
    this_metadata[k] = this_metadata[k][:512]
