...
sim_truevals = [
    sim_alphaV,
    sim_alphaH,
    sim_beta2 * sim_gain * -1e6,
    sim_beta3 * sim_gain**2 * -1e10,
    sim_beta4 * sim_gain**3 * -1e15,
    sim_gain,
    sim_ipnl,
    sim_ipnlNN,
    sim_omega,
...
def read_summary(fname):
    """Extract parameters for plots in Freudenburg et al. (2020)."""
    ...
    table["ipnlNN"] = (
        1e6 * (table["ipnl(0,1)"] + table["ipnl(0,-1)"] + table["ipnl(1,0)"] + table["ipnl(-1,0)"]) / 4.0
    )
    table["ipnl(0,0)"] *= 1e6
    table["alphaH"] *= 100
    table["alphaV"] *= 100
...
def read_visinfo(fname):
    ...
    table["bfevisNN"] = (
        1e6
        * (table["bfevis(0,1)"] + table["bfevis(0,-1)"] + table["bfevis(1,0)"] + table["bfevis(-1,0)"])
        / 4.0
    )
    table["phiNN"] = (
        1e6 * (table["phi(0,1)"] + table["phi(0,-1)"] + table["phi(1,0)"] + table["phi(-1,0)"]) / 4.0
    )
    table["phi(0,0)"] *= 1e6
    return table


font = {"family": "normal", "weight": "regular", "size": 13}

matplotlib.rc("font", **font)

prefix = "/users/PCON0003/cond0088/Projects/detectors/sw_outputs/"
summary_files = [
    prefix + "PaperIII/chris_20663st_summary.txt",
    ...
    prefix + "PaperIV_chargediffusion/chris_20829vis_fid1_summary.txt",
...
ylabels = [
    ...
    "SCA 20663, 128x16",
    "SCA 20663, cubic CNL",
    "SCA 20663, lo (1 3 4 6)",
    "SCA 20663, short (5 7 8 10)",
    "SCA 20663, med (3 6 7 10)",
    "SCA 20828, fiducial",
    "SCA 20828, 128x16",
    "SCA 20828, cubic CNL",
    "SCA 20828, lo (1 3 4 6)",
    "SCA 20828, short (5 7 8 10)",
    "SCA 20828, med (3 6 7 10)",
    "SCA 20829, fiducial",
    "SCA 20829, 128x16",
    "SCA 20829, cubic CNL",
    "SCA 20829, lo (1 3 4 6)",
    "SCA 20829, short (5 7 8 10)",
    "SCA 20829, med (3 6 7 10)",
    "SCA 20829, visible",
]
...
for i, ax in enumerate(axes):
    ...
    for j, _ in enumerate(ylabels):
        ...
        # visinfo values
            ...
            if visinfo_tables[j] is not None:
        ...
        if j == 18:  # i.e. if vis run # legacy # noqa: SIM108
            clr = "darkblue"
        else:
        ...
        if j == 0:
            for _, div in enumerate(divisions):
                ax.axhline(y=div + 0.5, color="k", linewidth=0.75, linestyle="-")

        if j % (divisions[1] - divisions[0]) == 0 and j != 18:
            if j == 0 or j == 6:
                ax.fill_betweenx(
                    [j - 0.5, j + divisions[1] - divisions[0] - 0.5],
                ...
                ax.fill_betweenx(
                    [j - 0.5, j + divisions[2] - divisions[1] - 0.5],
                    [mean - error, mean - error],
                    [mean + error, mean + error],
                    color="grey",
                    alpha=0.5,
...
plt.savefig("paramplot_p4.pdf", format="pdf")
plt.show()
