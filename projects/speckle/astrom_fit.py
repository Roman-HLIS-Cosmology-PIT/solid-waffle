...
for line in content:
    ...
    if m:
        buildRange = [int(m.group(1)), int(m.group(2)), int(m.group(3)), int(m.group(4))]
...
if len(sys.argv) > 2:
    ...
    outstem += f"_Row{thisrow:02d}"
    buildRange[0] = thisrow
    buildRange[1] = thisrow + 1
    if len(sys.argv) > 3:
        buildRange[1] = int(sys.argv[3]) + buildRange[0]
...
SCAMask[:, :, -4:] = 1
SCAMask[:, :4, :] = 1
SCAMask[:, -4:, :] = 1  # reference pixels

# Tell the user what we learned
print("Using files:")
for k in range(n_input_group):
    print("   ", inputdata[k], "repeats:", nrepeat[k])
print("max. number of repeats =", max_nrepeat)
print(f"number of superpixels: {ny:d} (in y) x {nx:d} (in x) = {nx*ny:d} (total)")
print("flat fields in", cflat)
print("power spectrum subregions", use_subregion, downsample)

# Check build ranges
if buildRange is None:
    buildRange = [0, ny, 0, nx]
print(f"building range: {buildRange[0]:3d}<=ty<{buildRange[1]:3d}, {buildRange[2]:3d}<=tx<{buildRange[3]:3d}")
...
if cflat:
    with fits.open(cflat) as hdul:
        cflat_image = hdul[0].data
    ...
    for k in range(n_input_group):
        ...
        del block
        del block2
    cflat_image = numpy.where(numpy.bitwise_or(cflat_image < 0.5, cflat_image > 1.5), 1.0, cflat_image)
...
for x in range(3):
    print(
        "    bit",
        x,
        numpy.sum(1 - numpy.prod(numpy.where(numpy.bitwise_and(SCAMask, 1 << x) == 0, 1, 0), axis=0)),
    )
print("")
...
for k in range(n_input_group):
    for ik in range(nrepeat[k]):
        print("FT FILE:", re.sub(r"\*", f"{ik+startInput:03d}", inputdata[k]))
        map = astromutils.read_segment(re.sub(r"\*", f"{ik+startInput:03d}", inputdata[k])) / cflat_image
        ...
        for yblock in range(downsample):
            for xblock in range(downsample):
                PS_downsample[k, yblock, xblock, :, :] = (
                    PS_downsample[k, yblock, xblock, :, :]
                    + numpy.abs(
                        numpy.fft.fft2(
                            map[
                                yblock * nside // downsample : (yblock + 1) * nside // downsample,
                                xblock * nside // downsample : (xblock + 1) * nside // downsample,
                            ]
                        )
                        ** 2
                    )
...
for k in range(n_input_group):
    PS_smooth[k, :, :] = convolve(
        PS[k, :, :], numpy.full((smooth_length, smooth_length), 1.0 / smooth_length**2), mode="wrap"
...
for k in range(n_input_group):
    ...
    print(f"total variance in group {k:2d} is {var:9.6f}, per superpix {varsub:9.6f}")

# and dx map
dxmap = numpy.zeros((astromutils.nTemplate, nside, nside))
# and dx map with filtering
sigmaprior = numpy.asarray([1e-4, 0.1, 0.1, 1e-4, 1e-4, 1e-4])
...
for ty in range(buildRange[0], buildRange[1]):
    ...
    for tx in range(buildRange[2], buildRange[3]):
        ...
        if use_subregion:
            PS_smooth = PS_downsample[:, yblock, xblock, :, :]
        CovWT = numpy.zeros((n_input_group, dy * dx, dy * dx))
        for k in range(n_input_group):
            print(f"Computing covariance matrix for group {k:2d} ...")
        ...
        for cy in range(niter):
            ...
            for a in range(astromutils.nTemplate):
                dNLLdp[a, :, :] = -dxmap[a, ymin:ymax, xmin:xmax] / sigmaprior[a] ** 2

            for k in range(n_input_group):
                print(
                    f"group {k:2d} [{ymin:4d}:{ymax:4d},{xmin:4d}:{xmax:4d}] cy{cy:4d}"
                    f" at {time.asctime(time.localtime(time.time())):s}"
                )

                # C,a-templates for this group
                dCdp = numpy.zeros((astromutils.nTemplate, dy * dx, dy * dx))
                for a in range(astromutils.nTemplate):
                    dCdp[a, :, :] = astromutils.getCovar((dy, dx), PS_smooth[k, :, :], branch=(a, -1))
                # (will add more later)
                # get the C^-1
                ...
                if cy > 0:
                    ...
                    for a in range(astromutils.nTemplate):
                        d_thisC = (
                            astromutils.getCovar((dy, dx), PS_smooth[k, :, :], branch=(a, -1))
                            * numpy.tile(xi[a, :], (dy * dx, 1)).T
                        )
                ...
                for ik in range(nrepeat[k]):
                    data = (
                        astromutils.read_segment(
                            re.sub(r"\*", f"{ik+startInput:03d}", inputdata[k]),
                            bounds=[ymin, ymax, xmin, xmax],
                        )
                        / norm[k, ik]
                        / cflat_image[ymin:ymax, xmin:xmax]
                    )
                    data = data / (1.0 + qnorm[ty, tx]) - 1.0
                ...
                for ik in range(nrepeat[k]):
                    ...
                    data = data / (1.0 + qnorm[ty, tx]) - 1.0 - datasum
                    Cid = Cinv @ data.flatten()
                    if ik == 0:
                        CiCthisCi = Cinv @ thisC @ Cinv
                    for a in range(astromutils.nTemplate):
                        G = Cid * (dCdp[a, :, :] @ Cid)
                        if ik == 0:
                            G -= (nrepeat[k] - 1) * numpy.sum(dCdp[a, :, :] * CiCthisCi, axis=1)
                ...
                for a in range(astromutils.nTemplate):
                    ...
                    for b in range(a + 1):
                        if a == b or not DiagFisher:
                            Fisher[a * dy * dx : (a + 1) * dy * dx, b * dy * dx : (b + 1) * dy * dx] += (
                                cprod[a, :, :] * cprod[b, :, :].T
                                + Cinv * astromutils.force_sym_mult(cprod[a, :, :], dCdp[b, :, :].T)
            ...
            if not DiagFisher:
                for a in range(astromutils.nTemplate - 1):
                    for b in range(a + 1, astromutils.nTemplate):
                        Fisher[a * dy * dx : (a + 1) * dy * dx, b * dy * dx : (b + 1) * dy * dx] = Fisher[
                            b * dy * dx : (b + 1) * dy * dx, a * dy * dx : (a + 1) * dy * dx
                        ].T
            ...
            if ForceFlat:
                ...
                Fisher[any_good2d] += 1.0 / flatprior**2 / (dy * dx) ** 2

            this_change = numpy.zeros((astromutils.nTemplate, dy, dx))
            for a in range(astromutils.nTemplate):
                w = (
                    Fisher[a * dy * dx : (a + 1) * dy * dx, a * dy * dx : (a + 1) * dy * dx]
                    + numpy.identity(dy * dx) / sigmaprior[a] ** 2
                )
            ...
            if cy == 0:
                dxfiltmap[:, ymin:ymax, xmin:xmax] = this_change  # report first change map
            if ChebIter:
                this_change2 = this_change / (
                    lmax
                    * (1.0 + alphaCheb * numpy.cos(numpy.pi / nCheb * ((cy * kCheb) % nCheb + 0.5)))
                    / 2.0
                )
            else:
