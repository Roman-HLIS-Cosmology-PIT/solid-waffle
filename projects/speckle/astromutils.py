...
def read_segment(infile, bounds=None):
    """
    Read a segment from the speckle test setup.
    ...
    bounds : list of int, optional
        If given, the bounds in the order [ymin, ymax, xmin, xmax]; otherwise read the whole image.

    Returns
    -------
    np.array
        The 2D image (either HDU #0 if there is just the primary HDU, or the difference between the
        first and last timestamps if there is a 4D HDU as well).
    ...
    if use_fitsio:
        ...
        if len(fileh) == 2:
            ...
            map[:, :] = fileh[1][0, 0, ymin:ymax, xmin:xmax].astype(numpy.float64) - fileh[1][
                0, nt - 1, ymin:ymax, xmin:xmax
        ...
        if len(hdul) == 2:
            map[:, :] = hdul[1].data[0, 0, ymin:ymax, xmin:xmax].astype(numpy.float64) - hdul[1].data[
                0, -1, ymin:ymax, xmin:xmax
            ].astype(numpy.float64)
        hdul.close()
    return map
...
def getCovar(size, powerspec, branch=(-1, -1)):
    ...
    ----------
    size : (int, int)
        Tuple (ny,nx) representing the size and shape of array.
    powerspec : np.array
        The 2D array of the power spectrum of size (nyPS, nxPS), both even.
        Normalized so that the sum is the total variance.
    branch : (int, int), optional
        Tuple (alpha,beta) of integers, describing offsets;
        negative integer for data with no offset (default).

    Notes
    -----
    `powerspec` [jy,jx] corresponds to (jy/nyPS, jx/nxPS) cycles per pixel
    (this will crash if nyPS,nxPS <= 2 ny,2 nx -- results in pattern wrapping).
    ...
    if nyPS <= 2 * ny or nxPS <= 2 * nx:
        print(
            "astromutils.getCovar: size error: asked for subarray of size",
            size,
            "from gridded power spectrum of size",
            numpy.shape(powerspec),
        )
        exit()
...
def force_sym_mult(A, B):
    """
    Multiplication for big matrices, where we know the output will be symmetric.
    """
    ...
    for i in range(m):
        ...
        for j in range(i + 1):
            ...
            if j < i:
                C[jmin:jmax, imin:imax] = C[imin:imax, jmin:jmax].T
    return C


def syminv(A):
    """
    Inverse of A where we know the result is symmetric.
    """
