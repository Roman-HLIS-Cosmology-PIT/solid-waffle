...
for iy in range(ny):
    for ix in range(nx):
        ...
        if ymax > nside - 4:
            ymax = nside - 4
        print(f"processing super pixel {iy:d},{ix:d}", f"--> [{ymin:4d}:{ymax:4d},{xmin:4d}:{xmax:4d}]")
        wx = xmax - xmin
        wy = ymax - ymin  # widths
        ...
        for k in range(n_input_group):
            for ik in range(nrepeat[k]):
                hdul = fits.open(re.sub(r"\*", f"{ik+1:03d}", inputdata[k]))
                if len(hdul) == 1:
                    data[k, ik, :, :] = hdul[0].data[ymin:ymax, xmin:xmax]
                if len(hdul) == 2:
                    data[k, ik, :, :] = hdul[1].data[0, 0, ymin:ymax, xmin:xmax].astype(numpy.float64) - hdul[
                        1
                    ].data[0, -1, ymin:ymax, xmin:xmax].astype(numpy.float64)
                hdul.close()
        ...
        for k in range(n_input_group):
            ...
            print(f"{k:2d} {iy:3d} {ix:3d} {c0:6.4f} {cw:6.4f} {a1:10.4E} {a2/a1:7.5f} {res:9.4E}")
            # put in cube
            all_cpp[k, iy, ix] = c0
...
hdu = fits.PrimaryHDU(
    numpy.transpose(out_array, axes=(0, 2, 1, 3)).reshape((nslice * ny, n_input_group * nx))
)
...
for k in range(n_input_group):
    ...
    print(
        "{:7.5f} {:7.5f} {:7.5f}".format(
            numpy.mean(all_cpp[k, :, :]),
            numpy.sqrt(m2_50),
            (numpy.sqrt(m2_75) - numpy.sqrt(m2_25))
            / 1.349
            / numpy.sqrt(numpy.size(all_mtf2[k, :, :]) - 1.0)
