...
for li in config:
    ...
    m = re.search(r"^REPEAT\:\s*(\S+)", li)
    if m:
        nrepeat = int(m.group(1))

    # amplitude
    m = re.search(r"^AMPLITUDE\:\s*(\S+)", li)
    if m:
        Amp = float(m.group(1))

    # seed
    m = re.search(r"^RNGSEED\:\s*(\S+)", li)
    if m:
        rngseed = int(m.group(1))

    # output
    m = re.search(r"^OUTFILE\:\s*(\S+)", li)
    if m:
        outfile = m.group(1)

    # gain
    m = re.search(r"^GAIN\:\s*(\S.*)", li)
    if m:
    ...
    # IPC
    m = re.search(r"^ALPHA\:\s*(\S+)", li)
    if m:
        alpha = float(m.group(1))

    # direction, H or V (lower case OK)
    m = re.search(r"^DIR\:\s*(\w)", li)
    if m:
        dir = m.group(1).upper()

    # slit dimensions, projected onto detector
    # first the long dimension, then the short dimension, then the center spacing
    m = re.search(r"^SLITDIM\:\s*(\S.*)", li)
    if m:
        slitdim = re.split(r"\s+", m.group(1))
        for j in range(len(slitdim)):
            slitdim[j] = float(slitdim[j])

    # custom information
    m = re.search(r"^CUSTOM\:\s*(\S.*)", li)
    if m:
        customSettings = re.split(r"\s+", m.group(1))
        custom = int(customSettings[0])
        for k in range(1, len(customSettings)):
            customSettings[k] = float(customSettings[k])

    # custom information
    m = re.search(r"^OFFSET\:\s*(\S.*)", li)
    if m:
        offsetSettings = re.split(r"\s+", m.group(1))
        offsetPattern = int(offsetSettings[0])
        for k in range(1, len(offsetSettings)):
            offsetSettings[k] = float(offsetSettings[k])
...
if custom > 0:
    ...
    dir = "X"
    slitdim = [300, 100, 100]  # avoid error for custom slit
if dir == "":
    print("Error: no DIR specified.")
    exit()
if len(slitdim) != 3:
    print("Error: wrong SLITDIM length:", len(slitdim), " <-- should be 3")
    exit()
if rngseed == 0:
    print("Error: no RNG seed specified.")
    exit()
if outfile == "":
...
if offsetPattern == 1:
    for i in range(512):
        for j in range(512):
            imOffset[1, 8 * j : 8 * (j + 1), 8 * i : 8 * (i + 1)] = -0.02
            if (i + j) % 2 == 1:
                imOffset[0, 8 * j : 8 * (j + 1), 8 * i : 8 * (i + 1)] = 0.02
if offsetPattern == 2:
    for i in range(512):
        for j in range(512):
            imOffset[1, 8 * j : 8 * (j + 1), 8 * i : 8 * (i + 1)] = -0.02
    ...
    for _ in range(32768):
        xc = numpy.random.randint(4, 4096)
        yc = numpy.random.randint(4, 4096)
        kmax = numpy.random.randint(1, 16)
        for k in range(kmax):
            imOffset[2, yc - 1, xc - k] = -0.02
            imOffset[2, yc, xc - k] = 0.02
        xc = numpy.random.randint(4, 4096)
        yc = numpy.random.randint(4, 4096)
        kmax = numpy.random.randint(1, 16)
        for k in range(kmax):
            imOffset[2, yc - k, xc - 1] = -0.02
            imOffset[2, yc - k, xc] = 0.02
    imOffset[3, :, :] = imOffset[1, :, :] ** 2
    imOffset[5, :, :] = imOffset[2, :, :] ** 2
    for _ in range(65536):
        xc = numpy.random.randint(8, 4088)
        yc = numpy.random.randint(8, 4088)
        imOffset[0, yc - 1 : yc + 2, xc - 1 : xc + 2] = -0.02
    for a in range(1, 6):
        imOffset[a, :, :] *= 1.0 + imOffset[0, :, :]
imOffset[:, :, :4] = 0.0
imOffset[:, :, -4:] = 0.0
imOffset[:, :4, :] = 0.0
imOffset[:, -4:, :] = 0
...
for krepeat in range(1, nrepeat + 1):
    print("generating " + outfile + f"_{krepeat:03d}.fits", "...")
    ...
    if custom == 1:
        ...
        for j in range(3):
            theta = 2.0 * numpy.pi * (j / 3.0 + 1.0 / 24.0)
            ...
            E_FT[yc : yc + L, xc : xc + L] = numpy.random.normal(size=(L, L)) + 1j * numpy.random.normal(
                size=(L, L)
            )
            for jj in range(L):
                for ii in range(L):
                    if (jj - L // 2) ** 2 + (ii - L // 2) ** 2 >= (L // 2) ** 2:
                        E_FT[yc + jj, xc + ii] = 0.0
    if custom == 2:
        # tilted slit
        ...
        for j in range(T):
            for i in range(T):
                ...
                if (xx < -L) or (xx > L) or (yy < -W) or (yy > W):
                    E_FT[j, i] = 0.0
    if custom == 3:
        # hexagon array
        # customSettings = [1, width, trianglesidelength]
        L = int(customSettings[1] * N / nside)
        for j in range(7):
            theta = 2.0 * numpy.pi * (j / 6.0 + 1.0 / 24.0)
            xc = int(customSettings[2] * N / nside * (1 + numpy.cos(theta)) / numpy.sqrt(3))
            yc = int(customSettings[2] * N / nside * (1 + numpy.sin(theta)) / numpy.sqrt(3))
            if j == 6:
                xc = yc = int(customSettings[2] * N / nside)
            E_FT[yc : yc + L, xc : xc + L] = numpy.random.normal(size=(L, L)) + 1j * numpy.random.normal(
                size=(L, L)
            )
    ...
    for i in range(-ov * (N // 2) + 1, ov * N // 2):
        kpix = 2.0 * numpy.pi * float(i) / N
        if i == 0:
            SmoothVec[i] = 1.0
        else:
            SmoothVec[i] = numpy.sin(kpix / 2.0) / (kpix / 2.0)
    ...
    ImageNoNoise = ImageNoNoise * (1.0 + imOffset[0, :, :]) + (
        imOffset[1, :, :]
        * numpy.real(numpy.fft.ifft2(1j * Intensity0 * kx))[0 : ov * nside : ov, 0 : ov * nside : ov]
        + imOffset[2, :, :]
        * numpy.real(numpy.fft.ifft2(1j * Intensity0 * ky))[0 : ov * nside : ov, 0 : ov * nside : ov]
        + imOffset[3, :, :]
        * numpy.real(numpy.fft.ifft2(-0.5 * Intensity0 * kx * kx))[0 : ov * nside : ov, 0 : ov * nside : ov]
        + imOffset[4, :, :]
        * numpy.real(numpy.fft.ifft2(-1 * Intensity0 * kx * ky))[0 : ov * nside : ov, 0 : ov * nside : ov]
        + imOffset[5, :, :]
        * numpy.real(numpy.fft.ifft2(-0.5 * Intensity0 * ky * ky))[0 : ov * nside : ov, 0 : ov * nside : ov]
    )
    ImageNoNoise = numpy.where(ImageNoNoise > 0, ImageNoNoise, 0.0)
    ...
    for i in range(nside):
        ImageNoise[:, i] /= gain[(i * nc) // nside]

    # quantize
    ImageNoise = numpy.where(ImageNoise > 0, ImageNoise, 0.0)
    ImageNoise = numpy.where(ImageNoise < 65535, ImageNoise, 65535.0)
    ...
    hdr["RNGSEED"] = rngseed
    hdr["IPCALPHA"] = alpha
    fitsio.write(outfile + f"_{krepeat:03d}.fits", ImageOut, header=hdr, clobber=True)
