...
def test_histogram(tmp_path):
    ...
    for i in range(1, N + 1):
        ...
        for j in range(nread):
            a = 1.0 + (x + y) / 8192.0
            sig = 2000.0 + 0.5 * y + 1000.0 * j + a * (20.0 * np.cos(x / 100.0) + 15.0 * np.sin(y / 10.0))
    ...
    histograms_main(
        [None, "-f", "1", "-i", str(tmp_path) + "/20260220T_001.fits", "-o", outfile, "-n", str(N)]
    )
    data = np.loadtxt(outfile).astype(np.int32)
    diff = data[2555:2566, 2] - np.array(
