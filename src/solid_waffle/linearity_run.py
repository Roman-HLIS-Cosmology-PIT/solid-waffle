...
def build_linearity_file(infile):
    ...
    for i in range(ncblock_use):
        ...
        for j in range(nramps):
            ...
            print(
                "   Smin deciles:", [numpy.round(numpy.percentile(Smin, 10 * ip), 1) for ip in range(1, 10)]
            )
            print(
                "   Smax deciles:", [numpy.round(numpy.percentile(Smax, 10 * ip), 1) for ip in range(1, 10)]
            )
        ...
        # clip
        Smin = numpy.clip(Smin, 0, 65534)
        Smax = numpy.clip(Smax, 0, 65534)  # <-- update to avoid accepting 65535 as unsaturated
        ...
        coef[1, :, :] = 1.0
        v = -numpy.linalg.solve(
            numpy.transpose(Amat[2:, 2:, :, :], axes=(2, 3, 0, 1)),
            numpy.transpose(Amat[1, 2:, :, :], axes=(1, 2, 0)),
        )
        ...
        for j in range(nramps):
            ...
            for tt in range(numpy.shape(cube)[0]):
                ...
                this_err = numpy.where(numpy.abs(z) < 1, this_err, 0.0)
