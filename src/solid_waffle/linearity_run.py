...
def build_linearity_file(infile):
    """
    Main routine to build the linearity file.
    ...
    def get_median_cube(flist, fileformat=None, xmin=None, xmax=None, tstart=None, tend=None):
        ...
        cube = np.zeros((tend - tstart + 1, nside, dx))
        for t in range(tstart, tend + 1):
            temp = np.zeros((nside, dx, len(flist)), dtype=np.float32)
            for k in range(len(flist)):
                temp[:, :, k] = pyirc.load_segment(flist[k], fileformat, [xmin, xmax, 0, nside], [t], False)
            cube[t - tstart, :, :] = np.median(temp, axis=-1)
        if sign > 0:
            cube = 65535 - cube  # flip to upward-going ramp
        return cube
    ...
    image_all = np.zeros((p_order + 6, nside, nside))

    # full error array, indicating specific ramps
    err_cube = np.zeros((nramps, nside, nside), dtype=np.uint16)

    # pixel flats
    pflat = np.zeros((nramps, nside, nside), dtype=np.float32)
    ...
    for i in range(ncblock_use):
        ...
        flists = []
        ntslice = np.zeros((nramps,), dtype=np.int16)
        for j in range(nramps):
            ...
            for k in range(pars["RAMPS"][j]["NRAMP"]):
                thisname = (
                    prefix + f"/{stamp:d}" + body + "_{:03d}.fits".format(k + pars["RAMPS"][j]["START"])
                )
        ...
        for j in range(nramps):
            print("Strip", i, "files -->", j, ntslice[j], flists[j])
            sys.stdout.flush()
            cube = get_median_cube(
                flists[j],
                fileformat=pars["RAMPS"][j]["FORMAT"],
                xmin=xmin,
                xmax=xmax,
                tstart=pars["RAMPS"][j]["TSTART"],
                tend=ntslice[j],
            )
            print(np.median(cube, axis=(1, 2)))
            Smin_ = np.amin(cube, axis=0)
            Smax_ = np.amax(cube, axis=0)

            # mask later parts of the ramp if they saturate
            halfframe = (np.shape(cube)[0] + 1) // 2
            diff = (cube[halfframe, :, :] - cube[0, :, :]) / halfframe
            for k in range(np.shape(cube)[0] - 2, p_order, -1):
                # diff = (Smax_-Smin_)/(np.shape(cube)[0]-1) # <-- old criterion
                if sign > 0:
                    cuthere = cube[k + 1, :, :] - cube[k - 1, :, :] < 2 * slopecut * diff
                    Smax_ = np.where(cuthere, cube[k, :, :], Smax_)
                else:
                    cuthere = cube[k + 1, :, :] - cube[k - 1, :, :] > 2 * slopecut * diff
                    Smin_ = np.where(cuthere, cube[k, :, :], Smin_)

            if j == 0:
                Smin = Smin_
                Smax = Smax_
            else:
                Smin = np.minimum(Smin, Smin_)
                Smax = np.maximum(Smax, Smax_)
            print("   Smin deciles:", [np.round(np.percentile(Smin, 10 * ip), 1) for ip in range(1, 10)])
            print("   Smax deciles:", [np.round(np.percentile(Smax, 10 * ip), 1) for ip in range(1, 10)])

        # set some more quality flags
        LowSignalRange = np.abs(Smax - Smin) < 1024
        dq[:, xmin:xmax] |= np.where(LowSignalRange, 2**20, 0).astype(np.uint32)

        # now extend range to cover the bias level
        if sign > 0:
            Smin = np.minimum(Smin, image_all[p_order + 2, :, xmin:xmax] - float(pars["NEGATIVEPAD"]))
        else:
            Smax = np.maximum(Smax, image_all[p_order + 2, :, xmin:xmax] + float(pars["NEGATIVEPAD"]))

        # clip
        Smin = np.clip(Smin, 0, 65534)
        Smax = np.clip(Smax, 0, 65534)  # <-- update to avoid accepting 65535 as unsaturated
        ...
        Amat = np.zeros((p_order + 1 + 2 * nramps, p_order + 1 + 2 * nramps, nside, dx))
        # build matrices
        for j in range(nramps):
            ...
            for tt in range(np.shape(cube)[0]):
                z = -1.0 + 2.0 * (cube[tt, :, :] - Smin) / (Smax - Smin)  # put in -1 .. +1
                mask = np.where(np.abs(z) < 1, 1, 0)  # if outside the range, mask that pixel
                u = np.zeros((p_order + 1, nside, dx))
        ...
        # check invertibility
        D = np.abs(np.linalg.det(np.transpose(Amat[2:, 2:, :, :], axes=(2, 3, 0, 1))))
        Dmed = np.median(D)
        NearSingularMatrix = 1e-12 * Dmed > D
        dq[:, xmin:xmax] |= np.where(NearSingularMatrix, 2**20, 0).astype(np.uint32)
        ...
        for l_ in range(2, p_order + 1 + 2 * nramps):
            Amat[l_, l_, :, :] += np.where(NearSingularMatrix, 1e24, 0)

        # now get the coefficients assuming we start with 0, 1
        coef = np.zeros((p_order + 1, nside, dx))
        coef[1, :, :] = 1.0
        v = -np.linalg.solve(
            np.transpose(Amat[2:, 2:, :, :], axes=(2, 3, 0, 1)),
            np.transpose(Amat[1, 2:, :, :], axes=(1, 2, 0))[..., None],
        )[..., 0]  # extra axis for NumPy 2 compatibility
        coef[2:, :, :] = np.transpose(v[:, :, : p_order - 1], axes=(2, 0, 1))

        # get linearization error
        err = np.zeros((nside, dx))
        err2 = np.zeros((nramps, nside, dx))
        for j in range(nramps):
            cube = get_median_cube(
                flists[j],
                fileformat=pars["RAMPS"][j]["FORMAT"],
            ...
            for tt in range(np.shape(cube)[0]):
                ...
                this_err = np.where(np.abs(z) < 1, this_err, 0.0)
                err = np.maximum(err, np.abs(this_err))
                err2[j, :, :] = np.maximum(err2[j, :, :], np.abs(this_err))

        # now do the rescaling to slope 1 at the bias point
        zbias = (image_all[p_order + 2, :, xmin:xmax] - Smin) / (
            Smax - Smin
        ) * 2.0 - 1.0  # z value at the bias
        slope = np.zeros((nside, dx))
        for L in range(1, p_order + 1):
            # this is to get dP_L(z)/dz, which is equal to sum_{L'<L, L-L' odd} (2L'+1) P_L'(z)
            dPl = np.zeros((nside, dx))
            for Lp in range((L - 1) % 2, L, 2):
                dPl += (2 * Lp + 1) * eval_legendre(Lp, zbias)
            slope += coef[L, :, :] * dPl
            del dPl
        slope *= 2.0 / (Smax - Smin)  # this is dz/dS
        ...
        print("   err deciles:", [np.round(np.percentile(err, 10 * ip), 1) for ip in range(1, 10)])

        # get the P-flat fields
        for j in range(nramps):
            pflat[j, :, xmin:xmax] = (v[:, :, p_order - 1 + 2 * j] / slope / tframe).astype(np.float32)

        del slope

        # and now the intercept
        icpt = np.zeros((nside, dx))
        for L in range(1, p_order + 1):
        ...
        for zstep in range(nzstep + 1):
            z = -1.0 + 2 * zstep / nzstep
            Sprime = np.zeros((nside, dx))
            for L in range(1, p_order + 1):
                # this is to get dP_L(z)/dz, which is equal to sum_{L'<L, L-L' odd} (2L'+1) P_L'(z)
                dPl = np.zeros((nside, dx))
                for Lp in range((L - 1) % 2, L, 2):
                ...
                Sprime += coef[L, :, :] * dPl
                del dPl
            dq[:, xmin:xmax] |= np.where(Sprime <= 0, 2**20, 0).astype(np.uint32)
        ...
        err_cube[:, :, xmin:xmax] = np.clip(np.round(err2), 0, 65535).astype(np.uint16)
    ...
    image_all[p_order + 5, :, :] = dq.astype(np.float32)
    ...
    for L in range(p_order):
        Im.header[f"SLICE{L+1:03d}"] = f"Legendre coef L={L:d}"
    Im.header[f"SLICE{p_order+1:03d}"] = "min signal in fit"
    Im.header[f"SLICE{p_order+2:03d}"] = "ref signal in fit (usually bias level; slope=1)"
    Im.header[f"SLICE{p_order+3:03d}"] = "max signal in fit"
    Im.header[f"SLICE{p_order+4:03d}"] = "max error"
    Im.header[f"SLICE{p_order+5:03d}"] = "quality"
    Im.header["QBIT20"] = "Bit 20: failed or unstable nonlinearity solution"
    Im.header["QBIT31"] = "Bit 31: ref pixel"

    # do the dark subtraction
    dark = np.zeros((nside, nside), dtype=np.float32)
    ...
    for j in range(32):
        print(f"flag {j:2d}, count {np.count_nonzero(dq&(2**j)):7d}")
    ...
    tree = {
        "roman": {
            "meta": {
                ...
                "instrument": {"detector": "WFI{:02d}".format(int(pars["SCA"])), "name": "WFI"},
            ...
            "maxerr": np.clip(np.round(image_all[p_order + 4, :, :]), 0, 65535).astype(np.uint16),
            "ramperr": err_cube,
        },
        "notes": {
            "script": script,
...
if __name__ == "__main__":
    build_linearity_file(sys.argv[1])
