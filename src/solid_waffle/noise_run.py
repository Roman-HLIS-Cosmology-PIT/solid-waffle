...
def run_noise(arglist, verbose=False):
    ...
    * ``-ro`` : turn on reference output
    ...
    if verbose: print('argument list ->', arglist)
    ...
    for i in range(1, len(arglist)):
      if arglist[i] == '-f': fileformat = int(arglist[i+1])
      if arglist[i] == '-i': infile = arglist[i+1]
      if arglist[i] == '-o': outfile = arglist[i+1]
      if arglist[i] == '-n': nfile = int(arglist[i+1])
      if arglist[i] == '-t': t_init = int(arglist[i+1])
      if arglist[i] == '-cd': cds_cut = float(arglist[i+1])
      if arglist[i] == '-rh': rowh = int(arglist[i+1])
      if arglist[i] == '-tn': tnoiseframe = int(arglist[i+1])
      if arglist[i] == '-nch': nch = int(arglist[i+1])
      if arglist[i] == '-ro': refout = True # has reference output
    ...
    if verbose: print(numpy.shape(firstframe))
    for k in range(nfile):
    ...
    iqrarray = numpy.percentile(firstframe, 75, axis=0, method='linear') - numpy.percentile(firstframe, 25, axis=0, method='linear')
    ...
    for j in range(nband):
      ...
      cdsnoise[ymin:ymax,:] = numpy.percentile(bandData, 75, axis=(0,1), method='linear')\
    ...
    for j in range(nband):
      ...
      tnoise[ymin:ymax,:] = numpy.percentile(bandData, 75, axis=0, method='linear')\
                                - numpy.percentile(bandData, 25, axis=0, method='linear')
      del bandData
    ...
    for j in range(nband):
      ...
      if verbose: print('shape of bandPower:', numpy.shape(bandPower))
      bandPowerDiff = bandPower[:,:,:,64] - numpy.mean(bandPower[:,:,:,60:64], axis=3)
      if verbose: print('shape of bandPowerDiff:', numpy.shape(bandPowerDiff))
      #for i in range(N//nch): print('{:3d} {:12.6f}'.format(i, numpy.mean(bandPower[:,:,:,i])))
      acnband[j] = numpy.mean(bandPowerDiff)/(N//nch)/2. # divide by length; then by 2 to get per sample
      if verbose: print('band', j, ': ACN =', acnband[j], 'DN**2'); sys.stdout.flush()
      del bandData
    ...
    if verbose: print('overall ACN', acnnoise)
    ...
    for j in range(nband+extra_power_spectra):
      ...
      for r in range(N): mySeries[:,:,r*(N//nband+rowh):r*(N//nband+rowh)+N//nband] = bandData[:,:,r,:]
    
      bandPower = numpy.mean(numpy.absolute(numpy.fft.fft(mySeries, axis=2, norm='ortho'))**2, axis=(0,1))
      bandPower *= (1.+rowh*nband/N)/slength/2. # divide by length; then by 2 to get per sample instead of CDS
      if verbose: print('shape of bandPower:', numpy.shape(bandPower))
      PS[j,:] = bandPower
    ...
    u_pink *= scale
    c_pink *= scale
    if verbose: print('c_pink =', c_pink, 'u_pink =', u_pink)
    if refout:
      ru_pink *= scale
      if verbose:
    ...
    if verbose: print(numpy.shape(cdsFL))
    for k in range(nfile):
      if verbose: print('build CDS ', k); sys.stdout.flush()
    ...
    iqr_cdsFL = (numpy.percentile(cdsFL, 75, axis=0, method='linear') - numpy.percentile(cdsFL, 25, axis=0, method='linear')).astype(numpy.float32)
    ...
    for i in range(nfile):
      if verbose: print('cdsFL[i,:,:] =', cdsFL[i,:,:])
    ...
    if verbose: print('stdev of pca0 =', pca0_stdev)
    # clean up memory
      ...
      for k in range(nfile):
        for kb in range(nstep):
          bandData[k,kb,:,:] = pyirc.load_segment(filelist[k], fileformat, [xmin,xmax,0,nside], [t_init+2*kb  ], False).astype(numpy.float32)\
                               ...
                               + ( (k+.5)/nfile + (kb+.5)/nstep -1.) # uniformly distribute to avoid quantization
      extra[1,:,:] = ( numpy.percentile(bandData, 75, axis=(0,1), method='linear')\
                                - numpy.percentile(bandData, 25, axis=(0,1), method='linear') )/1.34896 / numpy.sqrt(2.)
