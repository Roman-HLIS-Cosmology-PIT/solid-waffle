...
def run_spr_reduce(arglist, verbose=False):
    ...
    * ``-nl=<summary file>`` : summary file for NL information
    ...
    In *dictionary* format, the options are specified as: ``arglist["IN"]`` for the input file; ``arglist["OUT"]`` for
    the output stem; and e.g. ``arglist["-nd"]`` for the remaining options (for options that don't have an "=" value in
    the command line, you can set e.g. ``arglist["-i"]=True``).
    ...
    if use_nl:
      ...
      for x in f:
        ...
        if m:
          ...
          if thisord>omax:
            raise ValueError('Error: increase omax={:d} to {:d}'.format(omax,thisord))
      if verbose: print('NL information ->', sum_ny, sum_nx, omax, colindex)
    ...
    if verbose: print('median signal = {:8.6f} DN'.format(numpy.median(medsignals[1,:,:])))
