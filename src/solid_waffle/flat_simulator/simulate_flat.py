...
class Simulation:
    ...
    def __init__(self, cfg, verbose=True):
        ...
        for line in cfg.splitlines():
            ...
            if m:
                substep = int(m.group(1))

            # Time step (s)
            m = re.search(r"^DT:\s*(\S+)", line)
            if m:
                delta_tsamp = float(m.group(1))
    ...
    def run(self, verbose=False):
        ...
        nt_step = (
            self.pars["tsamp"] * self.pars["substep"]
        )  # number of tot timesteps depending on convergence needs
            ...
            if tdx == 1:
                ...
                if self.pars["bfemode"] == "true":
                    ...
                    area_defect = calc_area_defect(
                        (1.0 + self.pars["QY_omega"]) / (1.0 - self.pars["QY_omega"]) * a_coeff,
                        allQ[idx - 1, xmin:xmax, ymin:ymax],
                    )
                    meanQ = (
                        area_defect
                        * mean
                    ...
                    allQ[idx, xmin:xmax, ymin:ymax] += RNG.poisson(
                        ...
                        * (1.0 - self.pars["QY_omega"])
                        / (1.0 + self.pars["QY_omega"]),
                        allQ[idx, xmin:xmax, ymin:ymax].shape,
                    )
                    allQ[idx, xmin:xmax, ymin:ymax] += PairPoisson(
                        self.pars["QE"] * mean * self.pars["QY_omega"] / (1.0 + self.pars["QY_omega"]),
                        allQ[idx, xmin:xmax, xmin:xmax].shape,
        ...
        if self.pars["nlmode"] == "quadratic":
            data_cube_Q[:, xmin:xmax, ymin:ymax] -= (1.0e-6 * self.pars["nlbeta"]) * data_cube_Q[
                :, xmin:xmax, ymin:ymax
            ] ** 2
            if verbose:
            ...
            data_cube_Q[:, xmin:xmax, ymin:ymax] += (
                1.0e-6 * self.pars["nlcoeffs_arr"][0] * data_cube_Q[:, xmin:xmax, ymin:ymax] ** 2
                + 1.0e-6 * self.pars["nlcoeffs_arr"][1] * data_cube_Q[:, xmin:xmax, ymin:ymax] ** 3
                + 1.0e-6 * self.pars["nlcoeffs_arr"][2] * data_cube_Q[:, xmin:xmax, ymin:ymax] ** 4
            )
            if verbose:
