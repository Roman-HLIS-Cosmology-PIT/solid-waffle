...
def ipc_kernel_HV(alpha_H=0.01, alpha_V=0.01):
    ...
    alpha_H : float, optional
        The horizontal IPC coupling.
    alpha_V : float, optional
        The vertical IPC coupling.
...
def calculate_ipc(data_cube_Q, ipc_list, npad=2):
    ...
    for tdx in range(data_cube_Q.shape[0]):
        Q_pad = np.pad(data_cube_Q[tdx, :, :], pad_width=(npad, npad), mode="symmetric")
        ...
        data_cube_Q[tdx, :, :] = Q_pad_ipc[extra_dim:-extra_dim, extra_dim:-extra_dim]
    return data_cube_Q
...
class TestKernels:
    ...
    def get_bfe_kernel_3x3(cls):
        ...
        bfe_kernel_3x3 = 1.0e-6 * np.array([[0.065, 0.23, 0.065], [0.24, -1.2, 0.24], [0.065, 0.23, 0.065]])
    ...
    def get_bfe_kernel_5x5_ir(cls):
        ...
        bfe_kernel_5x5 = 1.0e-6 * np.array(
            [
        ...
        if np.sum(bfe_kernel_5x5) >= 1.0e-9:
...
def ipc_invkernel_HV(alpha_H=0.01, alpha_V=0.01):
    ...
    kernel = np.zeros((3, 3))
    kernel[0, 1] = kernel[2, 1] = -2.0 * alpha_H
    kernel[1, 0] = kernel[1, 2] = -2.0 * alpha_V
...
def a_symmetric_avg(coeffs, round=None):
    ...
    onezero1 = np.mean((coeffs[2, 3], coeffs[2, 1], coeffs[3, 2], coeffs[1, 2]))
    oneone1 = np.mean((coeffs[1, 1], coeffs[3, 1], coeffs[3, 3], coeffs[1, 3]))

    twozero1 = np.mean((coeffs[2, 0], coeffs[0, 2], coeffs[4, 2], coeffs[2, 4]))
    twoone1 = np.mean(
        (
            coeffs[4, 3],
            coeffs[3, 4],
    ...
    twotwo1 = np.mean((coeffs[0, 0], coeffs[0, 4], coeffs[4, 0], coeffs[4, 4]))

    means = np.array((zerozero1, onezero1, oneone1, twozero1, twoone1, twotwo1))
    if round is not None:
